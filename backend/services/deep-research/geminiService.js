...
const parseJsonFromGeminiResponse = (text) => {
...
export const determineAgentConfiguration = async (researchTopic) => {
  if (!ai) {
...
3.  Name: "Technological Advancement Agent", Focus: "Explore cutting-edge AI research (e.g., new algorithms, hardware) that could revolutionize healthcare.", Temperature: 0.7`;
  ...
  try {
    ...
    parsedJson.agents.forEach((agent, index) => {
...
export const runResearchAgent = async (researchTopic, agentConfig) => {
  if (!ai) {
    throw new Error("Gemini service not initialized. Call initializeGeminiService first.");
  }
...
// Add a simple generateContent method for compatibility
...
// Create module exports for compatibility
