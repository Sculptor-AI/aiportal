...
class TaskManager {
  ...
  async processTask(taskId) {
    ...
    try {
      ...
      const initialAgentStatuses = dynamicAgentConfigs.map((agent, index) => ({
  ...
  updateAgentStatuses(taskId, statuses) {
    const task = this.tasks.get(taskId);
    if (task) {
  ...
  updateSingleAgentStatus(taskId, agentId, status, message, research, sources) {
    const task = this.tasks.get(taskId);
    if (task) {
  ...
  completeTask(taskId, finalReport, sources) {
    const task = this.tasks.get(taskId);
    if (task) {
  ...
  updateTaskWithError(taskId, error) {
    const task = this.tasks.get(taskId);
    if (task) {
      task.status = 'error';
      task.error = error;
